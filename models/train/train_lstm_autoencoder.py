"""
LSTM Autoencoder for Phase 1 benchmark.

Reads:  D:/updated_dataset/models/windows/sequence_windows_all.npz
Writes: D:/updated_dataset/models/weights/lstm_autoencoder/lstm_best.pt
        D:/updated_dataset/models/weights/lstm_autoencoder/lstm_config.json
        (appends to model_scores_all.csv, model_metrics_all.csv)

Encoder: stacked LSTM over time. Decoder: stacked LSTM reconstructing sequence.
"""
import sys
import json
import time
...
class LSTMAutoencoder(nn.Module):
    ...
    Encoder: LSTM reads sequence, outputs hidden state.
    Decoder: LSTM reconstructs sequence from repeated hidden state.
    """
    ...
    def forward(self, x):
        ...
        # Repeat last hidden output T times as decoder input
        # h_n: (num_layers, B, hidden_size) — use top layer
        context = h_n[-1].unsqueeze(1).repeat(1, T, 1)  # (B, T, hidden_size)
        dec_out, _ = self.decoder(context, (h_n, c_n))
        x_hat = self.output_proj(dec_out)  # (B, T, F)
        return x_hat
...
def train_lstm():
    ensure_all_dirs()
    t0 = time.time()
    weights_dir = WEIGHTS_DIR / "lstm_autoencoder"

    npz = SEQ_ALL_NPZ
    if not npz.exists():
        print("  LSTM: sequence_windows_all.npz not found — SKIP")
    ...
    print(f"  LSTM: n_time={n_time}, n_feat={n_feat}, "
          f"train={len(tr_loader.dataset)}, val={len(va_loader.dataset)}")

    model = LSTMAutoencoder(n_feat, hidden_size=64, num_layers=2, dropout=0.1)
    ...
    model_path = weights_dir / "lstm_best.pt"
    ...
    pd.DataFrame({"train_loss": tr_losses, "val_loss": va_losses}).to_csv(
        weights_dir / "lstm_loss_curve.csv", index=False)
    ...
    with open(weights_dir / "lstm_config.json", "w") as f:
    ...
    print(f"  LSTM: F1={metrics['f1']:.4f} P={metrics['precision']:.4f} "
